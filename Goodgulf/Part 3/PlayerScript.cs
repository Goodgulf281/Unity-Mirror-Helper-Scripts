...
namespace Goodgulf.Networking
{
    ...
    {
        private TMP_Text messageText;                   // Text at the top right of the screen used for messages
        ...
        void Awake()
        {
            ...
            // Find the UI object used to display game announcement message (currently only Player Death)
            GameObject messageTextObject = GameObject.Find("textMessage");
            if(messageTextObject)
            {
        ...
        // This even is triggered by the Health script OnDeath event
        public void PlayerDied(PlayerScript playerScript)
        {
            // Find the unique identity for this player
            NetworkIdentity networkIdentity = playerScript.gameObject.GetComponent<NetworkIdentity>();

            // Send a remote procedure call to all clients (below method), pass the Network ID of the player who died
            PlayerDeath(networkIdentity.netId);
        }

        [ClientRpc]
        void PlayerDeath(uint netID)
        {
            // The server uses a Remote Procedure Call (RPC) to run that function on clients.
            ...
            {
                if (playerScript.netId == netID && playerScript.isLocalPlayer)
                {
                    // We found the player who died and it's the local player
                    if(messageText)
                        messageText.text="You died!";
                }
                else if (playerScript.netId == netID)
                {
                    // We found the player who died and it's another player
                    if (messageText)
                        messageText.text = "Player " + playerScript.playerName + " died.";
