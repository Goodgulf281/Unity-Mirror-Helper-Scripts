    ...
    public class UnityGameServer
    {
            ...
            [HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)] HttpRequest req, ILogger _logger)
        {
            ...
            if (command == "RegisterServer")
            {
                ...
                // (I didn't properly use en environment variable here so I commented out teh conenction string for the Storage Table)
                var serviceClient = new TableServiceClient(""); //Removed the Storage Account connection string
